...
RSpec.describe Releases::ReleaseIngestor do
  ...
  let(:raw_recordings) do
    JSON.parse(
      File.read(Rails.root.join("spec/fixtures/musicbrainz/recording_search/the_hop.json"))
    )
  end
  ...
  let(:candidate) do
    Musicbrainz::ReleaseCandidateExtractor.call(raw_recordings).first
  end
